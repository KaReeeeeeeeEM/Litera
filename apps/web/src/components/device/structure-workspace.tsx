...
function StructurePageEditor({ book, onResection, onSave, page }: { book: DeviceBook; onResection: () => void; onSave: (page: StructuredPage) => void; page: StructuredPage }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[22rem_minmax(0,1fr)]"><Card><CardHeader><CardTitle className="flex items-center gap-2"><ImageIcon/>Source page and media</CardTitle><CardDescription>Visual reference stays beside the editable reading structure.</CardDescription></CardHeader><CardContent>{imageUrl ? <div className="relative aspect-[3/4] overflow-hidden rounded-xl border bg-muted/20"><Image alt={`Source page ${page.pageNumber}`} className="object-contain" fill sizes="352px" src={imageUrl} unoptimized/></div> : <div className="grid aspect-[3/4] place-items-center rounded-xl border border-dashed text-sm text-muted-foreground">No page image available</div>}</CardContent></Card><div className="min-w-0"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-lg font-semibold">Reading order</h3><p className="text-sm text-muted-foreground">Edit text, roles, and image descriptions in sequence.</p></div><div className="flex gap-2"><Button onClick={onResection} variant="outline"><RefreshCw data-icon="inline-start"/>Re-section page</Button><Button onClick={() => { onSave({ ...draft, structuredAt: new Date().toISOString() }); toast.success(`Page ${page.pageNumber} structure saved.`); }}><Save data-icon="inline-start"/>Save changes</Button></div></div><ScrollArea className="mt-5 h-[calc(100vh-22rem)] min-h-80 max-h-[46rem] pr-4"><FieldGroup>{draft.sections.map((section, index) => <Field className="rounded-xl border p-4" key={section.id}><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><FieldLabel>Section {index + 1}</FieldLabel><SearchableSelect className="sm:w-48" onValueChange={(kind) => updateSection(section.id, { kind: kind as StructuredSection["kind"] })} options={sectionKinds} placeholder="Search section roles…" value={section.kind}/></div><Textarea aria-label={`Section ${index + 1} text`} onChange={(event) => updateSection(section.id, { text: event.target.value })} value={section.text}/>{section.kind === "image" ? <><FieldLabel htmlFor={`${section.id}-alt`}>Image description</FieldLabel><Textarea id={`${section.id}-alt`} onChange={(event) => updateSection(section.id, { altText: event.target.value })} placeholder="Describe the purpose and meaningful visual information for readers who cannot see the image." value={section.altText ?? ""}/></> : null}</Field>)}</FieldGroup></ScrollArea></div></div>;
}
